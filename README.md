...
> ⚠️ **STUDY PROJECT. PREDICTIONS GENERATED BY THIS SYSTEM MUST NOT BE USED AS THE BASIS FOR REAL INVESTMENT DECISIONS.** ⚠️

> Runs automatically every weekday after markets close — 20:30 UTC (~22:30 Barcelona CEST) — via GitHub Actions.
...
Each model receives **33 features** in 5 groups, computed from historical price and volume data plus external market context:

**Group 1 — Technical indicators (15 features)**
...
**Group 2 — External context (5 features)**
...
**Group 3 — Multi-horizon momentum (4 features)**

| Feature | Description | Why it matters |
|---------|-------------|----------------|
| `ret_1m`, `ret_3m` | 21-day and 63-day returns | Medium-term trend strength. A stock up 20% over 3 months is in a different regime than one flat over the same period. |
| `ret_6m`, `ret_12m` | 126-day and 252-day returns | Longer trend context. 12-month return captures the annual cycle and separates secular winners from mean-reverting names. |

**Group 4 — 52-week extremes (2 features)**

| Feature | Description | Why it matters |
|---------|-------------|----------------|
| `high52w_dist` | Distance from 52-week high (fraction, always ≤ 0) | Mean-reversion / breakout signal. Price near the 52-week high often signals either momentum continuation or exhaustion. |
| `low52w_dist` | Distance from 52-week low (fraction, always ≥ 0) | Bounce / capitulation signal. Price near the annual low is a structural support zone that many algorithms react to. |

**Group 5 — Calendar and cross-asset (7 features)**

| Feature | Description | Why it matters |
|---------|-------------|----------------|
| `day_of_week` | Trading day of week (0 = Mon, 4 = Fri) | Day-of-week effects are well-documented: Monday gaps and Friday profit-taking create systematic patterns. |
| `month` | Calendar month (1–12) | Seasonal effects: January effect, sell-in-May, year-end tax-loss harvesting. |
| `is_options_expiry` | 1 if within 2 days of 3rd Friday (US options expiry) | Options expiry weeks have systematically different vol and directional dynamics ("max pain" pinning). |
| `btc_ret_1d` | Bitcoin 1-day return (T-1) | Crypto risk-on/risk-off proxy. When BTC drops sharply, growth stocks and risk assets tend to follow. |
| `gold_ret_1d` | Gold (GLD) 1-day return (T-1) | Safe-haven flow signal. Gold up on a market day signals defensive rotation. |
| `corr_spy_20d` | 20-day rolling correlation with SPY returns | Measures how much the asset moves with the market. A stock with low or negative beta to SPY needs different signals than a high-beta name. |
| `vwap_dist` | Distance from 20-day VWAP (volume-weighted average price) | Institutional price anchor. Many algorithms and funds buy below VWAP and sell above it, creating a gravitational effect around this level. |
...
The watchlist contains **543 tickers across 70 sectors** used as macro context. These are not held assets; they are training signals. Accuracy figures for watchlist tickers are structurally lower (less data history, no entry-price anchoring) and should never be mixed with portfolio accuracy. The system tracks and reports accuracy separately:
...
Every day after market close, the pipeline runs a **walk-forward backtest** over the last 30 trading days for all portfolio tickers:

1. For each of the last 30 trading days with a known outcome, train only on data strictly before that day.
2. Predict direction for that day.
3. Compare to the actual outcome.

This gives a daily-updated, look-ahead-free accuracy figure stored in `output/wfv_log.csv` (cumulative, never deleted) and `output/wfv_results.json` (latest run summary). Unlike the CV metrics shown during training, WFV has zero overlap between the training set and the evaluation window.

### Statistical significance

The system runs a one-sided **binomial test** (H₁: accuracy > 50%) per horizon after each validation cycle, with a 95 % Wilson score confidence interval. Results are stored in `output/significance.json` and appended to the daily Telegram message.

The test stays silent until **n ≥ 100 validated predictions** per horizon — below that threshold, any accuracy figure is statistically meaningless and should not be interpreted. At approximately 3–4 months of daily operation the first horizon should cross this threshold.
...
```
Mon–Fri 20:30 UTC (~22:30 Barcelona CEST / ~21:30 CET — 30 min after all markets close)
  │
...
**Why after markets close:** NYSE/NASDAQ close at 20:00 UTC. Running at 20:30 UTC (30 min buffer, requested by the maintainer to land closer to 22:00 Barcelona local time instead of after midnight) ensures the day's closing prices are available for all portfolio assets — US equities, European equities, and crypto — with all validation icons (✅/❌) already filled. Note: this is a shorter buffer than the pipeline used previously (2h); if closing-price staleness issues appear, widen it back.
...
- ✅ **Schedule moved to after US market close** — cron retargeted to 22:00 UTC (midnight Barcelona CEST); NYSE/NASDAQ close at 20:00 UTC so all tickers have their day-close price available at validation time — fixes missing ✅/❌ icons for US equities and crypto
